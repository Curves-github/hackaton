...
export function getMustUnusedUnit(units: Unit[]): Unit {
  const complitedPoolsCount = (id: u32): u32 =>
    getComplitedPoolsWithUnit(id).length;
  ...
  return units.reduce<UnitUses>((minUnit, unit) => {
    ...
    return completedPoolsCount < minUnit.uses
      ? { unit: unit, uses: completedPoolsCount }
      : minUnit;
...
export function getClosestByRate(units: Unit[], target: Unit): Unit {
  ...
  for (let i = 1; i < units.length; i++) {
    ...
    if (
      closest.id == target.id ||
      rateDistance(target, unit) < rateDistance(target, closest)
    ) {
      closest = unit;
    }
  }
